...
describe("Wall", () => {
  ...
  it("should create a wall with given lane and z", () => {
    const wall = createWall(idGen, 2, -60);
  ...
  it("should assign incrementing ids", () => {
    const w1 = createWall(idGen, 0, -60);
    const w2 = createWall(idGen, 1, -60);
    expect(w1.id).toBe(0);
    expect(w2.id).toBe(1);
  });

  it("should reset id counter", () => {
    createWall(idGen, 0, -60);
    idGen.reset();
    const w = createWall(idGen, 0, -60);
    expect(w.id).toBe(0);
  });
